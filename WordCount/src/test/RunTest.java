...
public class RunTest {
	...
	public static void main(String[] args) throws Exception{
       ...
       // FileSystem fs = FileSystem.get(new URI(INPUT_PATH), conf);
	    ...
	    //1.0:新建job并初始化，设置主类，设置一个用户定义的job任务名
	    Job job = Job.getInstance(conf, "word count"); 
	    //1.1:装载编好的程序，设置生成Jar包的class
	    ...
	    //1.2.x:为job设置Combiner类
	    job.setCombinerClass(ReduceTest.class);  
	    //1.3:为job设置Reducer类
	    job.setReducerClass(ReduceTest.class);  
	     
	    // 设置MapReduce输入输出键值对格式
	    //1.4:为job的输出数据设置Key类，key设置为text类型，相当于java中的string类型
	    job.setOutputKeyClass(Text.class);         
	    //1.5:为job输出设置value类，value设置为IntWritable，相当于java中的int类型
	    job.setOutputValueClass(IntWritable.class); 
	    
	    // 1.6:设置输入与输出路径，还可以增加对输入与输出文件格式的设置
	    FileInputFormat.setInputPaths(job, new Path("d:/mr_input")); 
	    FileOutputFormat.setOutputPath(job, new Path("d:/mr_output")); 

	    // 2.0:提交MapReduce任务允许，固定写法，并等待任务运行结束
